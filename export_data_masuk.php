...
$sqlExport = "SELECT pengajuan.*, master_desa.desa, data_kuasa.* FROM pengajuan INNER JOIN master_desa ON pengajuan.id_desa = master_desa.id_desa LEFT JOIN data_kuasa ON pengajuan.id_kuasa = data_kuasa.id_kuasa WHERE pengajuan.status_pengajuan = 'Awaiting' and pengajuan.tahun_pengajuan = '{$selectedYear}' ORDER BY pengajuan.id_pengajuan ASC";
...
if (mysqli_num_rows($resultExport) > 0) {
    ...
    $filename = 'Export_Data_Masuk_' . $selectedYear .'.xlsx';
